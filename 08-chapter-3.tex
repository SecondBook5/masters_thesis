...
% Niche-conditioned regulatory transport in premalignant progression.
% All numbers reconciled to the v2lowrank_source run (authoritative). Verified against:
%   Re_per_program_{edge}_v2lowrank_source.parquet, ladder_{edge}_v2lowrank_source.parquet,
%   transport_stability_{edge}.parquet, panin/*_v2lowrank_source.parquet, gate1/*.csv.
% NOTE: inline citation placeholders are provisional -- resolve to \cite before final.
% INTEGRITY (2026-07-28 update): the full-cohort LUAD comm run has now COMPLETED. sec:comm numbers
%   (n_cells 209,879/553,298; LR 583,547/657,529; chains 1.09M/1.32M) trace exactly to
%   comm/{edge}/summary.json (verified byte-identical to HPC). sec:niche_gating carries the REAL
%   per-receiver niche-stratum gradient (Rcond_by_niche_{edge}.csv); the monotonic gradients are now
%   significant vs a within-donor stratum-permutation null (JAK-STAT/WNT/TRAIL/p53 all p~5e-4;
%   robustness/robustness_summary.json). Ladder differences now carry PAIRED fold-level 95% CIs
%   (local vs shuffle Delta +0.08 [-1.10,+1.26]; robustness_summary.json). Spatial autocorrelation
%   (Moran's I, Getis-Ord) computed on the authoritative full-cohort w joined to spatial coords
%   (spatial/spatial_autocorr_by_stage.parquet). The retrained-shuffled-w confirmatory null for the
%   stable-count is the one experiment still pending (needs HPC GPU retrain); flagged in
%   sec:hypothesis-assessment, not asserted. Every inline number traces to a real v2lowrank
%   parquet/csv/json (audited).
%
% LOCATION NOTE (2026-07-28): the Results chapter prose now lives DIRECTLY in this file
% (08-chapter-3.tex), no longer \input from drafts/results.tex. drafts/results.tex is retired.

\chapter{Results}
\label{chap:results}

Niche-conditioned regulatory transport, and the context-residual estimand $R_{\text{cond}}$ defined in Chapter~\ref{chap:methods}, were estimated on the epithelial compartment of the LUAD progression series and, as a cross-disease calibration probe, on a graded pancreatic intraepithelial neoplasia (PanIN) series. Each result below is reported with its effect size, held-out uncertainty, and the pre-specified acceptance criterion from Chapter~\ref{chap:methods}. Where the pre-registered falsification logic returned a negative, that negative is reported forthrightly as the primary reading of the edge.

\section{The progression series is edge-supported by shared donors at every step except the invasion boundary}
\label{sec:cohort}

The LUAD atlas spans five morphological stages arranged as a progression ladder: Normal, atypical adenomatous hyperplasia (AAH), adenocarcinoma in situ (AIS), minimally invasive adenocarcinoma (MIA), and invasive LUAD. Donor coverage per stage is 23 Normal, 10 AAH, 12 AIS, 4 MIA, and 25 LUAD donors. Because niche-conditioned transport was estimated between adjacent stages, the operative quantity is the number of donors that contribute epithelium to \emph{both} ends of an edge, which sets how much of each estimate is anchored within-patient rather than across-patient (Table~\ref{tab:edge_support}). The early edges are supported: eight donors bridge Normal$\rightarrow$AAH and two bridge AAH$\rightarrow$AIS. The invasion boundary AIS$\rightarrow$MIA has zero shared donors, so any transport across it is necessarily estimated across patients, whereas MIA$\rightarrow$LUAD recovers four shared donors. The AAH$\rightarrow$AIS edge, on which the primary regulatory analysis was run, carries 29{,}712 AAH epithelial receivers and 48{,}688 AIS epithelial cells.
...
These stable programs establish that a reproducible, sign-consistent, program-level context residual exists under patient-held-out resampling. Whether this reproducible set also exceeds a retrained shuffled-context baseline at the estimand level is a stronger test that is not yet run and is deferred to Section~\ref{sec:hypothesis-assessment}; the claim at this point is reproducibility, not superiority over shuffle. Whether that residual reflects information carried by \emph{local}, cell-resolved niche identity---as opposed to coarser, specimen-level ecology---is a further and stronger question, and the falsification ladder that follows (Section~\ref{sec:results-ladder}) answers it in the negative. The ecological reason for that boundary, developed in Section~\ref{sec:niche_gating}, is that the niche variation carrying progression signal is largely specimen-scale rather than cell-resolved at the current spatial resolution.
...
\section{At the invasion boundary, transport sustains WNT while suppressing VEGF, but local context no longer adds predictive value}
\label{sec:invasion}
...
\section{The estimand transfers to pancreatic precursors and returns a correctly calibrated null}
\label{sec:panin}
...
The SCENIC and hdWGCNA results are concordant with the regulatory-transport signature, with the important qualification that they are not an independent modality. Both are computed from the same epithelial expression matrix as R\textsubscript{cond}; they are independent \emph{methods}---de-novo motif-pruned co-expression topology and weighted module-level gene-set enrichment---applied to shared data, not orthogonal measurements. Their directional agreement with R\textsubscript{cond} on FOS, RB1, JAK-STAT suppression, and stromal ECM remodeling is therefore best read as internal consistency across analytic lenses rather than external validation. The overlap is also not enriched beyond chance: of the $53$ stable or directionally-stable R\textsubscript{cond} transcription factors, the fraction that appear among the highest-out-degree SCENIC regulators does not exceed the hypergeometric expectation for a random TF set of the same size (enrichment $0.95$--$1.14\times$; $P = 0.35$ to $0.63$ across hub thresholds), because most scored transcription factors appear somewhere in the co-expression graph. The concordance is thus qualitative and directional, and is reported as such rather than as a statistical corroboration.

\section{An R\textsubscript{cond}-gated framework links intercellular signaling to context-driven regulatory programs}
\label{sec:comm}
...
\section{The principal conclusions are stable to modeling choices}
\label{sec:sensitivity}

The two load-bearing conclusions---a reproducible program-level context residual, and its failure to
resolve at the receiver-local scale---are robust to the estimation choices, and the supporting
diagnostics are summarized here. Reproducibility of the stable set is anchored at the donor level:
every program classed STABLE on the AAH$\rightarrow$AIS edge holds its sign in all five patient-held-out
folds and in a leave-one-donor-out resampling (sign consistency $1.0$), so no single specimen drives
the stable count. The estimation geometry is itself identifiable: the normalized matching-stability
diagnostic $D_N$ favors the regulatory subspace used for transport on both edges, and is most stable
on the pathway block (Section~\ref{sec:results-ladder}; Appendix~\ref{sec:app-c-coupling}). The grey-box
decomposition is credible but bounded, with the residual-to-structured velocity ratio $D_{\rho}\leq1$
in four of five folds; this ceiling is reported rather than hidden, and the per-program attribution is
interpreted only to that extent. The negative local-context result is the more thoroughly stressed
conclusion: it holds across all five folds, across both LUAD edges, against a specimen-level average,
against a stratum-matched shuffle, and against the variance decomposition that explains it, and it is
not rescued by any of these comparisons. The complete ladders, transport-stability tables, per-fold
estimand summaries, and stability atlas are provided in Appendix~\ref{sec:app-c-ladder} and
Appendix~\ref{sec:app-c-stability}.

\section{Hypothesis assessment}
\label{sec:hypothesis-assessment}

The results assemble into a single, consistent shape. A structured regulatory-transport estimand, selected by a pre-registered identifiability benchmark, recovers a small set of reproducible program-level context residuals on both LUAD edges---a coordinated p53/WNT/TRAIL/AP-1 axis at the preinvasive step and a WNT-sustained, VEGF-suppressed axis at invasion---that survive patient-level resampling and are directionally concordant with co-expression analyses of the same data. At the same time, and on the same data, the pre-registered falsification tests return a bounded negative: local, cell-resolved niche context is only suggestively (not significantly) better than self-only transport and is statistically equivalent to specimen-level and shuffled context at the endpoint, and a variance decomposition supplies the ecological reason---the niche variation that carries progression signal is overwhelmingly specimen-scale and stage-invariant at Visium resolution. The estimand is thus reproducible and interpretable at the program level while its resolution is bounded to the specimen ecological scale, and the pancreatic series confirms that the estimator calibrates to the correct null when a cohort cannot support patient-robust stability. Against this backdrop the three pre-specified hypotheses of Section~\ref{sec:hypotheses} resolve as follows.
...
the four-donor PanIN series the powered stability filter returns zero stable programs---the correct
null at this cohort size---while the directionally consistent programs recover the expected
inflammatory-to-proliferative shift. The estimand transfers and calibrates correctly, but PanIN is
not an independently powered validation of the LUAD program set.
...
What this establishes for the biological question that motivated the work---whether the tissue context
of a single premalignant biopsy carries progression-relevant information beyond the epithelial cell
itself---and what it does not, is taken up in Chapter~\ref{chap:discussion}.
