...
\begin{figure}[htbp]
  ...
  \end{subfigure}
  \caption[Single-cell UMAP of the AAH$\rightarrow$AIS cohort]{Uniform manifold
  approximation and projection (UMAP) embedding of the single-cell transcriptomes making up the
  AAH$\rightarrow$AIS cohort, the edge on which the primary regulatory-transport analysis was run.
  \textbf{(a)}~Cells colored by progression stage: AAH (blue) and AIS (orange) cells occupy broadly
  overlapping regions of the embedding, indicating that the two precursor stages share overall
  cellular composition and are not separable by embedding position alone. \textbf{(b)}~The same cells
  colored by their fine cell-type annotation, spanning the full epithelial (AT1, AT2, basal, ciliated,
  secretory), immune (T, B, NK, macrophage, mast), stromal (fibroblast, CAF, myofibroblast), and
  endothelial compartments; the AAH and AIS epithelial \emph{receivers} that enter transport
  ($29{,}712$ and $48{,}688$ cells) are drawn from the epithelial populations of this atlas. The
  panels establish the cohort analyzed throughout the chapter and show that the stage distinction is
  subtle at the whole-transcriptome level, motivating the model-based, program-resolved transport
  estimand rather than a raw embedding contrast.}
...
% (task #53, plot_autocorr_summary / Getis-Ord, k=15) that produced the thesis numbers. If a Moran's-I
% trend panel is wanted, stage the per-section autocorr_summary/getis figure -- do NOT use this one.
\begin{figure}[htbp]
  \centering
  ...
  \end{subfigure}
  \caption[Spatial organization of the AAH and AIS microenvironment]{Spatial organization of the
  tissue microenvironment across the AAH and AIS sections of the Visium cohort (tissue coordinates;
  one sub-panel per section, labeled by GEO sample identifier and stage). \textbf{(a)}~Local
  malignant-cell fraction per spot: malignant density is concentrated in focal, spatially contiguous
  foci rather than dispersed uniformly, and the foci are more pronounced in several AIS sections.
  \textbf{(b)}~BANKSY spatial-domain assignments: each section partitions into spatially coherent
  microenvironmental domains, confirming that the composition features are spatially structured rather
  than well-mixed within every section. These maps are a descriptive, per-section illustration of the
  spatial structure whose stage-wise quantification---the monotonic rise in malignant-density
  Moran's~$I$ from AAH through AIS to invasive disease, and the positive within-section autocorrelation
  of every composition feature---is reported in the text; they cover the AAH and AIS sections only
  (the edge on which the primary analysis was run) and are not themselves a statistical test.}
...
\begin{figure}[htbp]
  ...
  \end{subfigure}
  \caption[Context difference field, both LUAD edges]{The niche-gated (context) component of transport rendered directly as the context difference field $v_{\text{full}}-v_{\text{self}}$ for (a)~the preinvasive AAH$\rightarrow$AIS edge and (b)~the AIS$\rightarrow$invasive edge. Within each panel the left plot draws, at every source receiver, the per-cell context redirection $v_{\text{full}}-v_{\text{self}}$---the full-context transport step minus the self-only step---as an arrow projected into the shared regulatory embedding and coloured by its magnitude $\|v_{\text{full}}-v_{\text{self}}\|$; the right plot recolours the same receivers by that magnitude relative to the median self-only step. Grey points are the real target-stage receivers and the faint backdrop is the surrounding tissue. This is the spatial rendering of the niche-gated term that the surrounding text distinguishes from the intrinsic progression field: the redirection is spatially coherent rather than random---a niche-conditioned component of transport is present---but its magnitude is small relative to the self-only step, consistent with the Helmholtz finding that adding context shifts the rotational (non-equilibrium) fraction by $\leq 0.011$ on every edge. The field is an illustrative, embedding-projected view of the context component, not an inferential test of it; the per-program significance of the context effect is quantified by $R_{\text{cond}}$ (Fig.~\ref{fig:forest}).}
...
\begin{figure}[htbp]
  ...
  \end{subfigure}
  \caption[Fitted regulatory velocity field along the progression axis]{The learned CRRT-UDE
  transport field, visualized as streamlines on the pooled receiver UMAP embedding for each pathological
  edge. Within each panel the self-only (context-neutral) and full-context (niche-gated) fields are drawn
  side by side over the source receivers at $\tau=0$ (blue) and the real target receivers (grey);
  streamline color and width encode local field speed. \textbf{(a)} The preinvasive AAH$\rightarrow$AIS
  edge and \textbf{(b)} the AIS$\rightarrow$invasive edge. The fields are directed rather than
  relaxational, and the self-only and full-context streamlines are near-indistinguishable on both edges,
  showing that the niche-gated term redirects transport only modestly---this is the field whose
  Helmholtz decomposition and rotational (non-equilibrium) fraction are quantified in the text. Because
  the fitted context effect is small by design, the redirection itself is shown directly as a difference
  field in Fig.~\ref{fig:context_diff_field} rather than being read off by subtracting these two
  near-identical panels by eye.}
