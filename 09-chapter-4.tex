...
\begin{figure}[htbp]
  ...
  fraction $\lVert v_{\text{rot}}\rVert/\lVert v\rVert$ (0 = gradient/equilibrium, 1 = rotational), for
  the self-only and full-context fields. \textbf{(a)}~preinvasive AAH$\rightarrow$AIS: $0.48$ vs.\ $0.49$
  ($+0.01$); \textbf{(b)}~AIS$\rightarrow$invasive: $0.77$ under both ($+0.00$). Context changes the
  fraction by $\leq 0.01$, so flux structure is a property of the intrinsic field, not the context
  perturbation; the fraction is far higher at invasion ($0.77$) than preinvasion ($0.48$). These are the
  grid-matched values paired with the Helmholtz decomposition (Fig.~\ref{fig:helmholtz-decomposition}).}
...
\begin{figure}[htbp]
  ...
  decomposition of the full (niche-gated) field on the receiver UMAP, per edge. Left: full field $v$;
  center: curl-free gradient part $-\nabla\Phi$ over the potential $\Phi$ (equilibrium component); right:
  divergence-free rotational part $v_{\text{rot}}$ (non-equilibrium flux). Suptitles give the flux
  fraction on the $28$-node grid ($0.53$ preinvasive, $0.76$ invasion; grid-matched values $0.48$/$0.77$
  in text, Fig.~\ref{fig:flux-index-self-full}). \textbf{(a)}~the preinvasive edge is more nearly
  gradient-like; \textbf{(b)}~invasion carries a larger rotational fraction. Descriptive of the fitted
  field's geometry---not evidence of a measured thermodynamic flux or biochemical limit cycle.}
...
\begin{figure}[htbp]
  ...
  \end{subfigure}
  \caption[Fitted potential landscape of the progression field]{The Helmholtz gradient potential $\Phi$
  (equilibrium component of Fig.~\ref{fig:helmholtz-decomposition}), sampled at every receiver and
  rendered as a Waddington-style surface over the PHATE embedding. \textbf{(a)}~AAH$\rightarrow$AIS and
  \textbf{(b)}~AIS$\rightarrow$invasive; invasion is less fully captured by this gradient component,
  consistent with its larger rotational residual. This is the potential of the \emph{fitted transport
  field}, distinct from the empirical $-\log$ density landscape of Fig.~\ref{fig:waddington}---neither is
  a measured free energy, and its coexistence with the flux is a geometric, not thermodynamic, property.}
