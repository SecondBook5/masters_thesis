...
\begin{figure}[htbp]
  ...
  \end{subfigure}
  \caption[Normalized transport-stability diagnostic by geometry, both edges]{Normalized matching-stability
  diagnostic $D_N$ (by target variance; lower = more stable barycentric targets) across candidate coupling
  geometries, for (a)~AAH$\rightarrow$AIS and (b)~AIS$\rightarrow$invasive; each bar annotated with its
  feature dimension $d$. This is the plotted companion to Table~\ref{tab:app-c-dn}---a quality-control
  diagnostic on the coupling geometry, not an inferential result: it ranks geometries by stability and
  supports estimating transport on the pathway-plus-TF geometry while reporting effects over the pathway
  sub-block. $D_N$ is a mini-batch quantity, not a patient-level confidence interval.}
...
\begin{figure}[htbp]
  ...
  \end{subfigure}
  \caption[Context-redirection streamfield, both LUAD edges]{Streamplot of the context difference field
  $\Delta v = v_{\mathrm{full}}-v_{\mathrm{self}}$ on (a)~AAH$\rightarrow$AIS and (b)~AIS$\rightarrow$invasive---a
  corroborating view of the per-receiver field in the main text (Fig.~\ref{fig:context_diff_field}). The
  redirection is smoothed onto the grid and drawn as streamlines coloured and scaled by $\|\Delta v\|$
  (clipped at the $95$th percentile), thinned to hairlines where negligible so a null region reads empty;
  blue points are source receivers, grey the targets. Coherent streamlines mark where niche context bends
  the flow. Descriptive, not inferential.}
...
\begin{figure}[htbp]
  ...
  \end{subfigure}
  \caption[Per-fold endpoint Sinkhorn distance, both LUAD edges]{Per-fold endpoint validation of the
  fitted full-context transport for (a)~AAH$\rightarrow$AIS and (b)~AIS$\rightarrow$invasive. Each
  patient-held-out fold shows the Sinkhorn distance between predicted and real target distributions
  (lower better), global beside conditional (within-niche, common-support-weighted, the supported
  estimand); bars annotated with per-fold donor and cell counts. These resolve the
  \texttt{local\_context\_full} row of the mean ladders (Tables~\ref{tab:app-c-ladder-aah},
  \ref{tab:app-c-ladder-inv}) and expose the fold-to-fold heterogeneity those means average over---why
  the paired context-versus-self contrast is underpowered on five folds. A stability diagnostic, not an
  independent test.}
...
\begin{figure}[htbp]
  ...
  \end{subfigure}
  \caption[Per-receiver $R_{\mathrm{cond}}$ distribution, PROGENy pathways, both edges]{Per-receiver
  $R_{\mathrm{cond}}$ for the $14$ PROGENy pathways on (a)~AAH$\rightarrow$AIS and
  (b)~AIS$\rightarrow$invasive, as horizontal violins over all out-of-fold receivers, ordered by
  $|\text{median}|$ and coloured by stability class (dashed line = no effect). Distributional complement
  to the main-text forest (Fig.~\ref{fig:forest}): a stable pathway is a tight violin displaced from
  zero, an unstable one straddles it. The context-upregulated p53/WNT/TRAIL at the preinvasive edge and
  the sustained-WNT/TRAIL, suppressed-VEGF pattern at invasion show as offset violins; the rest centre on
  zero.}
