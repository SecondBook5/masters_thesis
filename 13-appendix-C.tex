...
%% \todo{...} marks anything not verifiable from a file (see report). Compilable with the
...
% Local placeholder macro (no todonotes dependency); prints a visible red marker.
...
set of trained fields. The material is organized as fifteen sections: the dataset inventory
(C.1), preprocessing thresholds (C.2), the regulatory (C.3) and context (C.4) feature lists, the
patient-fold assignments (C.5), the edge-selector configurations (C.6), the full hyperparameter and
seed table (C.7), the coupling-geometry diagnostics (C.8), the training curves (C.9), the complete
falsification ladders for both LUAD edges (C.10), the program-stability atlas (C.11), the
spatial niche-association analysis (C.12), the invasion-edge supplement (C.13), the PanIN supplement
(C.14), and the software and reproducibility manifest (C.15).

% =====================================================================
\section{Dataset inventory}\label{sec:app-c-inventory}
...
% =====================================================================
\section{Preprocessing thresholds}\label{sec:app-c-preproc}
...
% =====================================================================
\section{Regulatory feature list}\label{sec:app-c-regfeatures}
...
% =====================================================================
\section{Context feature list}\label{sec:app-c-ctxfeatures}
...
% =====================================================================
\section{Patient-fold assignments}\label{sec:app-c-folds}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{ll}
    \toprule
    Edge (folds, donors) & Fold assignment \\
    \midrule
    AAH$\rightarrow$AIS ($5$ folds, $20$ donors)
      & \makecell[l]{fold 0: P8, P9, P11, P14; \quad fold 1: P20, P21, P23, P25;\\
                     fold 2: P1, P4, P12, P19; \quad fold 3: P16, P17, P22, P24;\\
                     fold 4: P2, P3, P5, P6} \\
    \midrule
    AIS$\rightarrow$invasive ($5$ folds, $25$ donors)
      & \makecell[l]{fold 0: P13, P15, P19, P20, P21, P23; \quad
                     fold 1: P1, P3, P5, P8, P11, P18;\\
                     fold 2: P2, P9, P10, P17, P25; \quad
                     fold 3: P6, P7, P12, P16;\\
                     fold 4: P4, P14, P22, P24} \\
    \midrule
    PanIN LG$\rightarrow$HG ($3$ folds, $7$ slides)
      & \makecell[l]{fold 0: PanIN-HG4, PanIN-LG2, PanIN-LG3; \quad
                     fold 1: PanIN-HG2, PanIN-HG3;\\
                     fold 2: PanIN-HG1, PanIN-LG1} \\
    \bottomrule
  \end{tabular}
\end{table}

% =====================================================================
\section{Edge-selector configurations}\label{sec:app-c-edges}
...
% =====================================================================
\section{Hyperparameters and seeds}\label{sec:app-c-hyperparams}
...
% =====================================================================
\section{Coupling and transport-geometry diagnostics}\label{sec:app-c-coupling}
...
degenerate reference point and is reported for completeness. The same diagnostic is plotted by candidate geometry for both edges in Figure~\ref{fig:app-c-transport-stability}.

\begin{figure}[htbp]
  \centering
  \begin{subfigure}[t]{0.49\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/ude_result/transport_stability_normalized_AAH_AIS.pdf}
    \caption{AAH$\rightarrow$AIS edge.}
    \label{fig:app-c-transport-stability-aah}
  \end{subfigure}
  \hfill
  \begin{subfigure}[t]{0.49\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/ude_result/transport_stability_normalized_AIS_invasive.pdf}
    \caption{AIS$\rightarrow$invasive edge.}
    \label{fig:app-c-transport-stability-inv}
  \end{subfigure}
  \caption[Normalized transport-stability diagnostic by geometry, both edges]{Normalized matching-stability diagnostic $D_N$ (normalized by target variance; lower indicates that the optimal-transport coupling yields more stable barycentric targets in that regulatory subspace) across the candidate coupling geometries, resolved separately for (a)~the AAH$\rightarrow$AIS edge and (b)~the AIS$\rightarrow$invasive edge. Each bar is annotated with the feature dimension $d$ of its geometry, making the $14$-pathway-versus-$759$-transcription-factor dimensional difference explicit. This is the plotted, per-edge companion to Table~\ref{tab:app-c-dn} (which additionally reports the raw and per-feature normalizations) and is an observational identifiability/quality-control diagnostic on the coupling geometry, not an inferential result about progression: it ranks the candidate geometries by transport stability and, with the table, supports estimating transport on the pathway-plus-transcription-factor geometry and reporting the interpretable pathway effects over the pathway sub-block. $D_N$ here is a mini-batch (batch-to-batch Monte-Carlo) quantity on this cohort rather than a patient-level confidence interval.}
  \label{fig:app-c-transport-stability}
\end{figure}
...
Figure~\ref{fig:app-c-credibility}. A streamplot rendering of the differential (context) transport geometry itself, $\Delta v = v_{\mathrm{full}}-v_{\mathrm{self}}$, is provided for both edges in Figure~\ref{fig:app-c-diff-streamfield}.

\begin{figure}[htbp]
  \centering
  \begin{subfigure}[t]{0.49\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/ude_result/difference_streamfield_AAH_AIS.pdf}
    \caption{AAH$\rightarrow$AIS edge.}
    \label{fig:app-c-diff-streamfield-aah}
  \end{subfigure}
  \hfill
  \begin{subfigure}[t]{0.49\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/ude_result/difference_streamfield_AIS_invasive.pdf}
    \caption{AIS$\rightarrow$invasive edge.}
    \label{fig:app-c-diff-streamfield-inv}
  \end{subfigure}
  \caption[Context-redirection streamfield, both LUAD edges]{Streamplot rendering of the context difference field $\Delta v = v_{\mathrm{full}}-v_{\mathrm{self}}$ on (a)~the AAH$\rightarrow$AIS edge and (b)~the AIS$\rightarrow$invasive edge---a second, corroborating view of the differential transport geometry shown per receiver in the main text (Fig.~\ref{fig:context_diff_field}). The per-receiver two-dimensional redirection $\Delta v$ (full-context step minus self-only step) is smoothed onto the streamplot grid and drawn as streamlines coloured and line-width-scaled by $\|\Delta v\|$ (clipped at the $95$th percentile); streamlines are thinned to hairlines where the redirection is negligible, so a genuine null region reads as an empty field. Blue points are source receivers ($\tau=0$), grey points the real target receivers, and the faint backdrop the surrounding tissue. Coherent, organized streamlines mark where local niche context bends the regulatory flow; a flat, low-magnitude field marks where it does not. Like the main-text field this is a descriptive rendering of the context component of transport, not an inferential quantity, and it is placed in the appendix to avoid duplicating the main-text illustration.}
  \label{fig:app-c-diff-streamfield}
\end{figure}
...
% =====================================================================
\section{Training curves}\label{sec:app-c-training}
...
overwritten. The per-stage convergence trace for each edge is shown in Figure~\ref{fig:app-c-training}.

\begin{figure}[htbp]
  \centering
  \begin{minipage}{0.49\textwidth}\centering
    \includegraphics[width=\linewidth]{figures/ude_result/training_convergence_AAH_AIS.pdf}\\
    {\footnotesize (a) AAH$\rightarrow$AIS}
  \end{minipage}\hfill
  \begin{minipage}{0.49\textwidth}\centering
    \includegraphics[width=\linewidth]{figures/ude_result/training_convergence_AIS_invasive.pdf}\\
    {\footnotesize (b) AIS$\rightarrow$invasive}
  \end{minipage}
  \caption[Three-stage training convergence]{Per-stage training loss for each LUAD edge. Stage~A
  (self-field) and Stage~B (context-field on the self-residual) converge over their allotted steps,
  and the Stage-C anchored joint refinement produces only a small further reduction---confirming that
  the self/context identification established in Stages A and B is preserved rather than overwritten in
  the joint step.}
  \label{fig:app-c-training}
\end{figure}
...
shown in Figure~\ref{fig:app-c-ladder-bars}. The per-fold endpoint Sinkhorn distances underlying these five-fold means are resolved by fold for both edges in Figure~\ref{fig:app-c-endpoint-by-fold}.

\begin{figure}[htbp]
  \centering
  \begin{subfigure}[t]{0.49\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/ude_result/endpoint_metrics_by_fold_AAH_AIS.pdf}
    \caption{AAH$\rightarrow$AIS edge.}
    \label{fig:app-c-endpoint-by-fold-aah}
  \end{subfigure}
  \hfill
  \begin{subfigure}[t]{0.49\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/ude_result/endpoint_metrics_by_fold_AIS_invasive.pdf}
    \caption{AIS$\rightarrow$invasive edge.}
    \label{fig:app-c-endpoint-by-fold-inv}
  \end{subfigure}
  \caption[Per-fold endpoint Sinkhorn distance, both LUAD edges]{Per-fold endpoint validation of the fitted full-context transport for (a)~the AAH$\rightarrow$AIS edge and (b)~the AIS$\rightarrow$invasive edge. For each patient-held-out fold the plot shows the Sinkhorn distance between the transported (predicted) target-stage distribution and the real target-stage distribution (lower is better), giving the global distance beside the conditional (within-niche, common-support-weighted) distance, which is the supported estimand; bars are annotated with the per-fold donor and cell counts. These are the per-fold resolutions behind the \texttt{local\_context\_full} row of the five-fold-mean ladders in Tables~\ref{tab:app-c-ladder-aah} and~\ref{tab:app-c-ladder-inv} and the mean-bar summary of Figure~\ref{fig:app-c-ladder-bars}. They expose the substantial fold-to-fold heterogeneity in endpoint distance that those means average over, which is the reason the paired local-context-versus-self-only endpoint contrast is underpowered on five folds. The figure is a per-fold stability diagnostic, not an independent test of the context effect.}
  \label{fig:app-c-endpoint-by-fold}
\end{figure}
...
$R_{\mathrm{cond}}$ spectrum across all $773$ programs is shown in Figure~\ref{fig:app-c-spectrum}. The per-receiver distribution of $R_{\mathrm{cond}}$ for the $14$ PROGENy pathways is shown for both edges in Figure~\ref{fig:app-c-re-violin}.

\begin{figure}[htbp]
  \centering
  \begin{subfigure}[t]{0.49\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/ude_result/Re_violin_pathways_AAH_AIS.pdf}
    \caption{AAH$\rightarrow$AIS edge.}
    \label{fig:app-c-re-violin-aah}
  \end{subfigure}
  \hfill
  \begin{subfigure}[t]{0.49\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/ude_result/Re_violin_pathways_AIS_invasive.pdf}
    \caption{AIS$\rightarrow$invasive edge.}
    \label{fig:app-c-re-violin-inv}
  \end{subfigure}
  \caption[Per-receiver $R_{\mathrm{cond}}$ distribution, PROGENy pathways, both edges]{Distribution of the per-held-out-receiver conditional-transport effect $R_{\mathrm{cond}}$ for the $14$ PROGENy pathway programs on (a)~the AAH$\rightarrow$AIS edge and (b)~the AIS$\rightarrow$invasive edge, drawn as horizontal violins with jittered points and a narrow box over all out-of-fold receivers, ordered by $|\text{median}|$ and coloured by cross-fold stability class; the dashed line marks no context effect ($R_{\mathrm{cond}}=0$). This is the distributional complement to the point-plus-confidence-interval forest of the main text (Fig.~\ref{fig:forest}) and to the all-$773$-program spectrum (Fig.~\ref{fig:app-c-spectrum}): a stable pathway appears as a tight violin displaced from zero, whereas an unstable one straddles zero. The stable, context-upregulated p53, WNT, and TRAIL pathways at the preinvasive edge, and the sustained-WNT/TRAIL with suppressed-VEGF pattern at the invasion edge, are visible as violins offset from zero, while the remaining pathways centre on it.}
  \label{fig:app-c-re-violin}
\end{figure}
...
\subsection{Descriptive tissue ecology across the preinvasive edge (provisional)}
\label{sec:app-c-ecology-provisional}
...
\subsection{IL1B-macrophage niche and context-residual transport (provisional)}
\label{sec:app-c-il1b-provisional}
...
The main text (Section~\ref{sec:comm}) defines the R\textsubscript{cond}-gated communication framework
...
OmniPath, retains only chains whose terminal TF program is \textsc{stable}/\textsc{directionally\_stable}
...
\paragraph{Differential network curvature.}
Ollivier--Ricci curvature $\kappa$ was computed on the weighted differential communication network for
each edge, following the construction of Section~\ref{sec:comm}; a negative curvature marks an edge
through which signaling mass is funneled (a structural bottleneck), and the per-channel difference
$\Delta\kappa = \kappa_{\text{target}} - \kappa_{\text{source}}$ quantifies bottleneck emergence across
the stage transition. On the AAH$\rightarrow$AIS edge, $1{,}588$ of $2{,}415$ channels ($66\%$) become
more bottlenecked at AIS (mean $\Delta\kappa = +0.089$; note the positive mean reflects a heavy right
tail of a few strongly reinforced channels against many mildly bottlenecked ones). The strongest
emerging bottlenecks are SMG-mucous$\rightarrow$$\gamma\delta$T ($\Delta\kappa=-0.49$),
myoepithelial$\rightarrow$NK-CD11d ($-0.28$), and a cluster of innate-lymphoid-cell channels
(ILC$\rightarrow$neuroendocrine $-0.26$, ILC$\rightarrow$SMG-duct $-0.24$, ILC$\rightarrow$secretory
$-0.24$); the most reinforced are muscle-smooth-airway$\rightarrow$SMG-mucous ($+3.19$) and
neuroendocrine$\rightarrow$SMG-mucous ($+2.72$), placing the submucosal-gland-mucous population at the
center of the reorganization (Figure~\ref{fig:ricci_diff}, main text). On the AIS$\rightarrow$invasive
edge the reorganization is more balanced ($1{,}366$ of $3{,}003$ channels, $45\%$, more bottlenecked;
mean $\Delta\kappa=+0.135$), and the emerging bottlenecks shift toward a stromal/fibroblast convergence
(NKT$\rightarrow$Schwann $-0.28$, and multiple $\rightarrow$perichondrial-fibroblast channels: ciliated
$-0.27$, CD4-effector $-0.23$, CD8-EM $-0.23$). These curvature summaries are descriptive properties of
the LIANA differential network and are not $R_{\mathrm{cond}}$-gated; they characterize how the
communication topology---independent of the estimand---reorganizes across each transition.
