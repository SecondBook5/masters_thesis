...
overwritten. The per-stage convergence trace for each edge is produced by the pipeline as
\texttt{training\_convergence.pdf}.
\todo{Copy \texttt{results\_hpc/figures/AAH\_AIS/ude\_result/training\_convergence.pdf} and the
AIS\_invasive equivalent into the thesis \texttt{figures/} directory and insert them here as a
two-panel figure; the files are not yet present in the template figure directory, so no
\texttt{\textbackslash includegraphics} is emitted (kept compilable).}
...
compartments in AAH versus AIS. \emph{These values are provisional}: they were computed on a
verification spatial-context table with only three donors per stage, which is underpowered for a
donor-level test (no feature clears a Benjamini--Hochberg $q<0.05$). The full-cohort spatial-context
table is pending; the directional trend---alveolar-macrophage abundance lower and malignant fraction
higher in AIS---is reported here for completeness and is not cited as a finding in the main text.

\begin{table}[htbp]
  \centering
  \caption[Tissue-ecology composition, AAH vs AIS (provisional)]{Donor-level mean compartment
  abundance in AAH vs AIS on the verification spatial-context table (3 donors/stage; underpowered,
  no feature clears $q<0.05$). Provisional pending the full-cohort table.}
  ...
  \begin{tabular}{lcccc}
    ...
    \midrule
    Alveolar macrophage & $0.070$ & $0.027$ & $-0.042$ & $0.021$ \\
    Macrophage (total)  & $0.038$ & $0.045$ & $+0.007$ & $0.814$ \\
    Fibroblast          & $0.105$ & $0.099$ & $-0.006$ & $0.803$ \\
    CAF fraction        & $0.121$ & $0.124$ & $+0.003$ & $0.935$ \\
    Immune fraction     & $0.294$ & $0.212$ & $-0.081$ & $0.248$ \\
    Malignant fraction  & $0.017$ & $0.067$ & $+0.050$ & $0.283$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
A pre-registered hypothesis of this work is that an inflammatory (IL1B-high) macrophage niche is
associated with the context-driven regulatory programs. Table~\ref{tab:app-c-il1b} reports a mixed
model of per-receiver $R_{\mathrm{cond}}$ on macrophage abundance (donor random intercept), and the
direction is coherent: the interferon/inflammatory programs JAK-STAT, NF$\kappa$B, and Hypoxia
increase their context effect with macrophage density. \emph{This result is provisional and
pseudoreplicated}: on the verification spatial-context table the receiver$\leftrightarrow$context
join covers only three donors, so the spot-level fixed-effect $p$-values are not donor-level
significant and effect sizes must be re-estimated on the authoritative full-cohort table before the
association is claimed. It is therefore reported here, not in the main text.

\begin{table}[htbp]
  \centering
  \caption[Macrophage niche vs context residual, AAH$\rightarrow$AIS (provisional)]{Mixed-model slope
  of per-receiver $R_{\mathrm{cond}}$ on macrophage abundance (per SD, donor random intercept) on the
  verification spatial-context table (3 donors). Direction coherent; provisional pending the
  full-cohort table.}
  ...
  \begin{tabular}{lcc}
    ...
    \midrule
    JAK-STAT & $+0.040$ & $[+0.035,\,+0.046]$ \\
    NF$\kappa$B  & $+0.025$ & $[+0.020,\,+0.030]$ \\
    Hypoxia  & $+0.020$ & $[+0.016,\,+0.025]$ \\
    p53      & $+0.013$ & $[+0.008,\,+0.017]$ \\
    EGFR     & $-0.010$ & $[-0.017,\,-0.003]$ \\
    \bottomrule
  \end{tabular}
\end{table}

\todo{Re-estimate Tables~\ref{tab:app-c-ecology} and \ref{tab:app-c-il1b} on the authoritative
full-cohort spatial-context table (\texttt{w\_AAH\_AIS.parquet}); if the macrophage$\rightarrow$
$R_{\mathrm{cond}}$ association holds at the donor level, promote it to the main-text Results biology
section. A finer module-level niche-gating analysis (per-module $\eta^2$ against a within-donor
permutation null) is likewise pending the authoritative run.}
...
provided for the SCENIC environment (\texttt{workflow/envs/locks/}) to allow exact recreation of that
stage.
\todo{The primary \texttt{crrt.yaml} environment is specified with loose version constraints; capture
the exact resolved versions of decoupleR, scvi-tools, PyTorch, torchdiffeq, POT, and geomloss from the
HPC \texttt{conda list} / lock file and pin them here for a byte-reproducible manifest.}
\todo{Record the git commit hash of the \texttt{crrt-ude} repository at which the
\texttt{v2lowrank\_source} results were produced (working-tree HEAD at analysis time was on branch
\texttt{ude-core}), so the manifest ties the numbers in this appendix to an exact revision.}
