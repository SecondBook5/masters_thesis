...
\graphicspath{{figures/}{figures/presentation/}{figures/grn/}{figures/scenic/}{figures/gsea/}{figures/comm/}}

% ---- fail-soft graphics -----------------------------------------------------
\makeatletter
\def\sf@dirs{,figures/,figures/presentation/,figures/grn/,figures/scenic/,figures/gsea/,figures/comm/}
...
\begin{frame}{One measurement, two disjoint readings}
  \centering\safefig[height=0.74\textheight]{fig1-panel-a}
\end{frame}
\note{%
...
\begin{frame}{Correspondence is estimated \emph{within} ecological strata}
  \centering\safefig[height=0.74\textheight]{fig1-panel-b}
\end{frame}
\note{%
...
\begin{frame}{The field is structured, and context enters through a waist}
  \centering\safefig[width=0.97\textwidth]{fig1-panel-c}
\end{frame}
\note{%
...
\begin{frame}{One start state, two integrations}
  \centering\safefig[width=0.92\textwidth]{fig1-panel-d}
\end{frame}
\note{%
...
\begin{frame}{The context vector does not encode the receiver}
  \vfill
  \begin{center}
  {\small ridge regression, ecological context $\rightarrow$ receiver state,
  held-out}\\[1.2em]
  {\Large\color{good}$R^2 < 0$ in all five folds}\\[.6em]
  {\small mean $-0.54$ \quad worse than predicting the mean}
  \end{center}
  \vfill
\end{frame}
\note{%
...
\begin{frame}{The preinvasive residual is a circuit, not a list}
  ...
  \begin{columns}[T]
    \begin{column}{0.46\textwidth}\centering
      {\color{bad}\textbf{Down}}\\[.5em]
      ETV3 \quad ETV6\\ ZBTB18 \quad GLI3\\[.5em]
      {\small\color{bad}four transcriptional repressors}
    \end{column}
    \begin{column}{0.46\textwidth}\centering
      {\color{good}\textbf{Up}}\\[.5em]
      FOS\\[.3em] p53 \quad RB1 \quad KAT5\\[.5em]
      {\small\color{good}their target, and the checkpoint}
    \end{column}
  \end{columns}
  \vfill
  \begin{center}\hl{Loss of repression under retained restraint.}\end{center}
  \vfill
\end{frame}
\note{%
...
\begin{frame}{Interferon tone is lost \emph{before} invasion}
  ...
  \begin{center}\begin{tabular}{lcc}
    \toprule & AAH $\rightarrow$ AIS & AIS $\rightarrow$ invasive\\
    \midrule
    JAK-STAT & \color{bad}$-0.246$ & ---\\
    IRF9 & \color{bad}$-0.129$ & ---\\
    \addlinespace
    IFN-$\gamma$ response & \color{bad}$-1.85$ & \color{good}$+1.78$\\
    IFN-$\alpha$ response & \color{bad}$-1.75$ & \color{good}$+1.80$\\
    \bottomrule
  \end{tabular}\end{center}
  \vfill
  \begin{center}\small two readouts, different confounds, same direction\end{center}
  \vfill
\end{frame}
\note{%
...
\begin{frame}{Invasion is a change of architecture, not more drive}
  ...
  \begin{columns}[T]
    \begin{column}{0.46\textwidth}\centering
      {\color{bad}\textbf{Down}}\\[.5em]
      HDAC1 \quad CTBP1 \quad NAB2\\[.5em]
      {\small\color{bad}three corepressors}
    \end{column}
    \begin{column}{0.46\textwidth}\centering
      {\color{good}\textbf{Up}}\\[.5em]
      MAML1 \quad FOXA1\\[.5em]
      {\small\color{good}coactivator and pioneer factor}
    \end{column}
  \end{columns}
  \vfill
  \begin{center}\hl{Lineage identity is dismantled here, not before.}\end{center}
  \vfill
\end{frame}
\note{%
...
\begin{frame}{An anabolic-stress module, on the same edge}
  ...
  \begin{columns}[T]
    \begin{column}{0.46\textwidth}\centering
      \textbf{Context residual}\\[.5em]
      ATF5 \quad HSF2 \quad NFKB2\\[.4em]
      {\small stress response and proteostasis}
    \end{column}
    \begin{column}{0.46\textwidth}\centering
      \textbf{Whole transcriptome}\\[.5em]
      MYC targets \quad OxPhos\\[.4em]
      {\small both reversed from preinvasive}
    \end{column}
  \end{columns}
  \vfill
  \begin{center}\hl{Anabolic demand and its compensation, together.}\end{center}
  \vfill
\end{frame}
\note{%
...
\begin{frame}{A split verdict --- and one half is a bound}
  \centering\safefig[height=0.40\textheight]{pres_ladder_AAH_AIS}
  \vfill
  \begin{center}\begin{tabular}{lcc}
    \toprule Paired contrast & $\Delta$ & 95\% CI\\ \midrule
    Local over \textbf{shuffled} & $+0.08$ & \color{good}$[-1.10,\,+1.26]$\\
    Local over \textbf{self-only} & $+8.76$ & \color{bad}$[-2.83,\,+20.35]$\\
    \bottomrule
  \end{tabular}\end{center}
\end{frame}
\note{%
...
\begin{frame}{Why one interval is tight and the other is not}
  ...
  \begin{center}\small\begin{tabular}{lccccc c}
    \toprule & F0 & F1 & F2 & F3 & F4 & SD\\ \midrule
    self $-$ local & $+5.82$ & $+22.04$ & $+14.26$ & $+2.73$ & \color{bad}$-1.06$ & $9.33$\\
    local $-$ shuffle & \multicolumn{5}{c}{\emph{folds co-move}} & $\approx1.0$\\
    \bottomrule
  \end{tabular}\end{center}
  \vfill
  \begin{center}\hl{Structural, not arbitrary.}\end{center}
  \vfill
\end{frame}
\note{%
...
\begin{frame}{At the invasion boundary, both comparisons are null}
  ...
  \end{tabular}\end{center}
  \vfill
  \begin{center}\small\hl{zero shared donors on this edge}\end{center}
  \vfill
\end{frame}
\note{%
