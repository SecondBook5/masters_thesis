...
\begin{frame}{The preinvasive residual is a circuit, not a list}
  ...
  \only<2>{%
    \centering\safefig[height=0.74\textheight]{scenic_rss_vs_rcond}\\[.1em]
    {\footnotesize the same signature over all $\sim$300 SCENIC regulons: FOS most
    context-elevated, IRF9/ETV6/SMAD4 most context-removed, and the stable set
    (dark) is what survives all five folds}}%
  \only<3>{%
    \centering\safefig[height=0.80\textheight]{regulon_network}\\[.1em]
    {\footnotesize $R_{\mathrm{cond}}$-weighted CollecTRI graph: node size $=|R_{\mathrm{cond}}|$,
    colour $=$ cross-fold stability, edges $=$ shared targets}}%
\end{frame}
\note{%
...
\cue{advance} And it is not a cherry-picked handful. Plotted over all three
...
\begin{frame}{Invasion is a change of architecture, not more drive}
  ...
  \only<2>{%
    \centering\safefig[height=0.74\textheight]{module_umap_inv}\\[.1em]
    {\footnotesize the actual co-expression architecture at this edge: seven modules,
    each anchored on a lineage hub gene}}%
  \only<3>{%
    \centering\safefig[height=0.80\textheight]{module_enrich_inv}\\[.1em]
    {\footnotesize and the module that carries ciliary / axonemal identity (SM7) is
    the one whose enrichment collapses at invasion}}%
\end{frame}
\note{%
...
\cue{advance} You can see that architecture directly. An independent co-expression
...
\cue{advance} And when I score those modules for functional enrichment, the
...
\begin{frame}{Progression is a directed flow, not a relaxation}
  ...
  \only<2>{\safefig[width=0.98\textwidth]{dyn_helmholtz_AAH_AIS}\\[.15em]
    {\footnotesize preinvasive edge --- gradient part is the landscape, rotational
    part is non-equilibrium flux; \hl{flux fraction $0.53$}}}%
  \only<3>{\safefig[width=0.98\textwidth]{dyn_helmholtz_AIS_invasive}\\[.15em]
    {\footnotesize invasion edge, same decomposition --- \hl{flux fraction rises to
    $0.76$}: the transition is markedly less gradient-like}}%
\end{frame}
\note{%
...
[build 2 --- preinvasive Helmholtz] Decompose the field into a gradient part (equilibrium,
the landscape) and a rotational part (non-equilibrium flux). On the preinvasive edge the
non-equilibrium fraction is $0.53$ --- already substantially rotational, not a clean
descent.

\cue{advance} [build 3 --- invasion Helmholtz] The same decomposition at the invasion edge:
the fraction rises to $0.76$. The invasion transition is the most strongly non-gradient ---
a directed, cycle-like flow, not relaxation to a fixed point. And adding context changes
this fraction by at most $0.01$ --- the non-equilibrium character is a property of the
intrinsic progression field, not of the niche.

\cue{this is the systems-biology beat --- ``landscape AND flux,'' the Wang framework.
Say the two numbers: 0.53 preinvasive, 0.76 invasion --- and now they SEE both.}
